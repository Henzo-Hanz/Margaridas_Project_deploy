...
O frontend deve consumir a api gerada pelo backend, retornando requisições para a aplicção e modificando os dados atravez de uma UI amigável e simplificada, não precisa ser uma aplicação individual com react ou algo do gênero, pode ser apenas Htmls que são puxados da pasta "templates" e que enviam infos através de forms para a Api.  

- Implementar o estilo do layout na homepage ("/")